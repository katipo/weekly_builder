...
module WeeklyHelper
  ...
  class WeeklyBuilder
          ...
          for day in @start_date..@end_date
            concat(tag("div", :id => @day_row))
              for event in @objects
                if event.starts_at.strftime('%Y %j').to_s == day.strftime('%Y %j').to_s
                  if event.starts_at.strftime('%H').to_i >= @start_hour and event.ends_at.strftime('%H').to_i <= @end_hour
                    concat(tag("div", :id => "week_event", :style =>"left:#{left(event.starts_at)}px;width:#{width(event.starts_at,event.ends_at)}px;", :onclick => (options[:with_onclick] ? "location.href='/events/#{event.id}';" : nil)))
    ...
    def width(starts_at,ends_at)
      #example 3:30 - 5:30
      return ((75 * (24 - starts_at.strftime('%H').to_i)) - 12) if ends_at.strftime('%H').to_i == 0
